...
ggplot(p1to7_all_long, mapping = aes(x = value, fill = stage, color = stage)) + facet_wrap(.~feature, nrow = 2, scales = "free")  + geom_line(stat = "density")+theme(legend.position = "bottom")+ guides(color=guide_legend(nrow=1,byrow=TRUE))+scale_color_manual(values=c("red","orange","magenta","green","blue","black"))

ggplot(p1to7_all_long, aes(x=stage, y=value, fill=stage)) + facet_wrap(.~feature, nrow = 2, scales = "free") +
  geom_boxplot(width = 0.8, outlier.size=0.4) +theme(legend.position = "bottom")+  scale_x_discrete(expand=c(0.12, 0))+ scale_fill_manual(values = c("red","orange","magenta","green","blue","black"))+ guides(fill=guide_legend(nrow=1,byrow=TRUE))
```
...
feature.p <- ggplot(data=df_long, aes(x=stage, y=value, fill = stage)) +
  geom_bar(stat="identity", width=0.8, show.legend=FALSE)  + facet_wrap(.~feature, nrow = 1, scales = "free")+theme(strip.text = element_text(size=7.5)) + scale_fill_manual(values = c("red","orange","magenta","green","blue","black"))
...
rate.p <- ggplot(data=df.rate_long, aes(x=stage, y=Increase.rate, group = 1)) +
  geom_point(aes(colour = factor(stage))) + geom_line() + facet_wrap(.~feature, nrow = 1) + theme(strip.text = element_text(size=7.5)) + scale_color_manual(values = c("red","orange","magenta","green","blue","black"))+ scale_y_continuous(limits = c(-0.5, 1.5)) + theme(legend.position = "none")
